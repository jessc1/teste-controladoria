...
class Transaction:
    ...
    def validate_side(self, value: str) -> Optional[str]:
        ...
        return self.SIDE_OPTIONS.get(value, None)
    ...
    def validate_row(self, row: Dict[str, Any], source_file: str) -> Tuple[Optional[Dict], Optional[Dict]]:
        ...
        try:
            ...
            client_doc, doc_clean, doc_type = self.validate_client_document(
    ...
    def read_transactions_files(self) -> pd.DataFrame:
        """Read all transaction files from the transactions directory."""
        all_data = []
    ...
    def process(self) -> bool:
        ...
        print("Reading transaction files...")
        ...
        for idx, row in transactions_df.iterrows():
            source_file = row.pop('_source_file') if '_source_file' in row else 'unknown'
            
            cleaned_row, error_row = self.validate_row(row.to_dict(), source_file)
        ...
        print(f"After deduplication: {len(self.transactions)}")
    ...
    def generate_clean_transactions(self):
        """Generate clean_transactions.csv with all valid transactions."""
    ...
    def generate_invalid_rows(self):
        """Generate invalid_rows.csv with all invalid transactions."""
    ...
    def format_currency(self, value: float) -> str:
        ...
        abs_val = abs(value)
        formatted = f"R$ {abs_val:,.2f}".replace(',', '#').replace('.', ',').replace('#', '.')
        return formatted
    
    def generate_daily_positions(self):
        """Generate daily_positions.xlsx with daily aggregations by ticker."""
        if not self.transactions:
        ...
        output_df = grouped[['date', 'ticker', 'gross_amount_formatted', 'avg_trade_price_formatted', 'total_costs_formatted']].copy()
        output_df.columns = ['date', 'ticker', 'gross_amount', 'avg_trade_price', 'total_costs']
